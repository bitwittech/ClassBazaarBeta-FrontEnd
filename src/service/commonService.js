...
export const eduTest = async (user, val) => {
    ...
    // e.preventDefault();
    const url = `https://edubuk.co.in/mitest/register?user_id=${user.id}&name=${val.name}&gender=${val.gender}&email_address=${val.email_address}&school_or_college_name=${val.school_or_college_name}&class_year=${val.class_year}&city=${val.city}&mobile_no=${val.mobile_no}&password=${val.password}`;
